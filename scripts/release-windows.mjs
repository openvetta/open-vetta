...
const desktopDir = join(repoRoot, "apps", "desktop-app");
...
test 读取 apps/desktop-app/.env.development，且必须显式指定版本；
...
async function main() {
	...
	runBun(["run", "--cwd", "apps/desktop-app", "dist:win:inno"], childEnvironment);
	if (options.channel === "stable") {
	...
	runBun(["run", "--cwd", "apps/desktop-app", "verify:updates:windows"], childEnvironment);
	...
	runBun(["run", "--cwd", "apps/desktop-app", "publish:updates:r2"], childEnvironment);
