...
- desktop-app 类型检查：仓库根目录 `bun run check` **已包含** `bunx tsc --noEmit -p apps/desktop-app/tsconfig.json`（在 Biome + monorepo `tsgo` 之后）。
- 单独排查时也可：`cd apps/desktop-app && bunx tsc --noEmit`，或在仓库根目录 `bunx tsc --noEmit -p apps/desktop-app/tsconfig.json`。不要在仓库根目录裸跑 `bunx tsc --noEmit`（那会用根 tsconfig，**查不到** desktop-app / i18n 等类型错误）。
