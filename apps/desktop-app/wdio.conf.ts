...
function resolveElectronBinaryPath(): string {
	...
	if (!electronBinary || !existsSync(electronBinary)) {
		throw new Error(
			`Electron binary not found (got: ${String(electronBinary)}). Install the electron dependency in apps/desktop-app.`,
		);
	}
...
function resolveElectronServiceOptions(): {
	...
	if (!existsSync(mainEntry)) {
		throw new Error(
			`Main-process build artifact missing: ${mainEntry}. Run bun run build in apps/desktop-app first (or at least build:main + preload + renderer).`,
		);
	}
...
export const config = {
	runner: "local",
	// Resolve from apps/desktop-app so monorepo root cwd does not matter.
	rootDir: packageRoot,
	specs: ["./e2e/**/*.e2e.ts"],
	exclude: [],
	maxInstances: 1,
