...
本文记录 `apps/desktop-app` 构建与打包阶段的体积优化策略。目标是减少发布包中不会被当前运行路径使用的文件，同时避免删除源文件、模型文件或依赖包内容。
...
```bash
cd apps/desktop-app && bun run build:ocr-runner
cd apps/desktop-app && bun run build:main
```
...
在 `apps/desktop-app` 下先完成对应平台的 `dist:*`，再从 CI 或发布机上传 `release/` 中的更新文件：
