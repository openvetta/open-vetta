 ...
 *   <apps/desktop-app>/resources/appshot/bin/Vetta Computer Use.app
 * which scripts/build-appshot-helper.js compiles as part of `bun run dev`.
 * We resolve relative to `process.cwd()`, which is `apps/desktop-app`
	...
	if (!existsSync(fullPath)) {
		throw new Error(
			`appshot helper binary not found at ${fullPath}. ` +
				`Run 'node scripts/build-appshot-helper.js' in apps/desktop-app, or rebuild Vetta.app.`,
		);
	}
