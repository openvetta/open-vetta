...
- 侧边栏打开已有会话时会立即切换选中态并静默禁用发送；历史快照改为通过只读文件通道优先渲染，再恢复 Runtime 状态与实时订阅。打开过渡不再被顶层路由骨架替换，右侧消息列表会持续挂载并直接展示完整 Markdown、工具调用、插件卡片和消息操作，不再用短暂提示或轻量消息替换造成布局抖动；Virtuoso 只在首屏收窄屏幕外预渲染范围，并在浏览器空闲时扩大。Runtime 历史只替换真正变化的消息，避免重复渲染已出现的行。快速连续点击采用 newest-wins，旧请求不再覆盖新会话；跨进程诊断日志可按 `interactionId` 关联 Renderer、React 提交、Main 与 Coding Agent 初始化阶段。
