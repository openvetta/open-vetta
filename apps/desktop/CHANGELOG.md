...
- 侧边栏改为按产品语义投影会话：使用会话自有工作空间的 Agent Team 会话与普通对话一起展示，选择项目的团队会话展示在对应项目下；团队行以成员叠放头像和团队名称标识，不再提供割裂的 Team 专属筛选列表。
...
- 修复 Agent Team 停止时成员完成通知仍可能重新唤起负责人、导致随后发送的消息被忙碌 Runtime 静默挂起的问题；停止现在会清理待续跑通知，停止后立即发送的新消息可以正常开始，竞态完成的成员任务也不会在停止后复活团队执行。
- 修复 Agent Team 成员完成任务后，其执行槽仍被负责人续跑占用的问题；负责人再次向同一成员委派任务时，新任务现在会立即从等待状态开始执行，不再与完成通知形成循环等待。
- 修复 Agent Team 运行中的工具回合在实时流与持久化快照短暂重叠时连续显示两次的问题；同一成员的同一回合现在始终复用唯一时间线身份。
