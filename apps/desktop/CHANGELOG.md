...
- 侧边栏打开已有会话时会立即切换选中态、显示“正在打开会话”并禁用发送，Runtime 恢复、历史与订阅完成后再进入可交互态；快速连续点击采用 newest-wins，旧请求不再覆盖新会话。跨进程诊断日志可按 `interactionId` 关联 Renderer、Main 与 Coding Agent 初始化阶段。
