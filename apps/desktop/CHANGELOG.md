...
- Agent Team 聚合会话现在以固定高度摘要卡展示成员的 thinking、工具阶段和回复状态，并通过稳定的 request/source turn 关联避免成员完成后卡片丢失或错位；成员乱序完成时摘要卡仍保持原委派顺序，点击卡片可进入对应成员会话。

- 统一新会话发送团队消息时，先保留会话 ID 和发送快照并立即切换到团队会话页，再由目标页在首帧后创建 Team 会话记录并发送，不再等待 Runtime 初始化或负责人处理完成才跳转；发送期间输入框保持可编辑，后续输入与已发送请求使用独立快照，不会被清空或修改。
...
- Agent Team 首条消息现在只等待本次实际寻址成员的 Runtime 就绪，其他成员继续后台预热。
