...
- **内置技能按构建形态裁剪**：`skills-manifest.json` 新增 `requiresCloud` 标记，lite 构建下依赖云服务的内置技能（`publish-ability` 发布能力）在技能列表、能力页与 Agent 装配中整体隐藏、不加载。内置 vetta MCP 本就在未登录时不注册，lite 下无需额外处理；推荐 MCP（Notion/Figma/GitHub 等第三方）不受影响。
