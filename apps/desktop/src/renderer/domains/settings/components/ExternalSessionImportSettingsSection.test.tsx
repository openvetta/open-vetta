...
function model(overrides: Partial<ExternalSessionImportSettingsModel> = {}): ExternalSessionImportSettingsModel {
	return {
		actions: {
			specifyGrokSessionDir: vi.fn(async () => undefined),
			toggleGrokEnabled: vi.fn(),
		},
		canSpecifyGrokPath: false,
		grokDisplayPath: "/Users/ada/.grok/sessions",
		grokEnabled: false,
		labels: {
...
describe("ExternalSessionImportSettingsSection", () => {
	it("以独立分区展示 Grok 开关和只读路径，不提供可编辑输入", async () => {
		const current = model();
		...
		expect(current.actions.toggleGrokEnabled).toHaveBeenCalledWith(true);
	});
	...
	it("仅在自动探测失败时出现手工指定入口", async () => {
		...
		expect(current.actions.specifyGrokSessionDir).toHaveBeenCalledOnce();
	});
});
