import { ComposerCapabilityRegion, composeComposerCapabilities } from "@shared/composer";
import type { ComposerCapability } from "@shared/composer";
...
export function InputBarView({ model, className, classNames }: InputBarViewProps): JSX.Element {
	...
	const composition = composeComposerCapabilities<InputBarComposerRegion, ReactNode>([
		capability("editor", "surface.editor", (
		...
		capability("surface-decoration", "surface.decoration", <ThemedInputBarBackground />),
		capability(
			"command-palette",
			"surface.command",
		...
		capability("attachment-preview", "surface.attachments", (
			<div
			...
			>
				<div className="min-h-0 overflow-hidden rounded-t-[inherit]">
					{renderCapsules ? (
						<div
							className={["space-y-1.5 px-3 pt-3", classNames?.capsules]
								.filter(Boolean)
								.join(" ")}
						>
							...
							{model.appshotAttachment ? (
								<AppshotCard
									data={model.appshotAttachment}
									onRemove={model.actions.removeAppshot}
								/>
							) : null}
	...
	return (
		...
		>
			...
			>
				<ComposerCapabilityRegion composition={composition} region="stack.before" />
				...
				>
					<ConversationComposer.Decoration>
						<ComposerCapabilityRegion composition={composition} region="surface.decoration" />
					</ConversationComposer.Decoration>
					<ConversationComposer.Content className={classNames?.cardContent}>
						<ConversationComposer.Command>
							<ComposerCapabilityRegion composition={composition} region="surface.command" />
						</ConversationComposer.Command>
						<ConversationComposer.Attachments>
							<ComposerCapabilityRegion composition={composition} region="surface.attachments" />
						</ConversationComposer.Attachments>
						<ConversationComposer.Editor>
							<ComposerCapabilityRegion composition={composition} region="surface.editor" />
						</ConversationComposer.Editor>
						<ConversationComposer.Toolbar>
							<ComposerCapabilityRegion composition={composition} region="surface.toolbar" />
						</ConversationComposer.Toolbar>
				 ...
				 */}
				<InputBarFooter
					items={composition.get("stack.footer").map((contribution) => ({
...
function capability(
	id: string,
	region: InputBarComposerRegion,
	value: ReactNode,
	requires?: readonly string[],
): ComposerCapability<InputBarComposerRegion, ReactNode> {
	return {
		id,
		...(requires ? { requires } : {}),
		contributions: [{ id: "view", region, value }],
	};
}
