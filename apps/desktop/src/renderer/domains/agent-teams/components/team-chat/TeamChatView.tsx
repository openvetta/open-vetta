...
export function TeamChatView({ model, actions }: TeamChatViewProps): JSX.Element {
	...
	const composition = composeComposerCapabilities<TeamComposerRegion, ReactNode>([
		capability(
			"team-routing",
			"routing",
			<TeamComposer
					...
					<div className="relative mx-auto w-full max-w-2xl">
						...
						>
							<ConversationComposer.Content>
								...
								{composition.get("attachments").length > 0 ? (
									<ConversationComposer.Attachments>
										<ComposerCapabilityRegion composition={composition} region="attachments" />
									</ConversationComposer.Attachments>
								) : null}
...
function capability(
	id: string,
	region: TeamComposerRegion,
	value: ReactNode,
	requires?: readonly string[],
): ComposerCapability<TeamComposerRegion, ReactNode> {
	return {
		id,
		...(requires ? { requires } : {}),
