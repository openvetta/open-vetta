/*
 * orbkit-core：WebGL 着色器球的运行时（画布、参数插值、状态预设）。
 *
 * 外部素材，通过 `bunx shadcn@latest add zzzzshawn/orbkit/shdr-25` 引入，除 import 路径外
 * 保持上游原样——要升级就重新 add 一次再改回路径，不要在这里手工改逻辑。
 * 对外只经 `OrbitOrb` 使用，不要在业务代码里直接引这两个文件。
 */
