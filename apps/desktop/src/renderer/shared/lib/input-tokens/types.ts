 ...
 * - skill 编辑态引用：`@skill:名字`（名字含空白或引号时 `@skill:"名字"`）；
 *   无 scene 时发送边界把第一个 skill 转成 `PromptRequest.promptRef` 硬展开
 ...
 *
 * scene 与 skill 共用编辑器 Token；发送边界会把 scene（或无 scene 时的第一个 skill）
 * 从正文剥离并转换为唯一的 PromptRequest.promptRef。scene 还会注入 tasks.json 生成的 todo。
 */
