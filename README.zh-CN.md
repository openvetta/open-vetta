...
Open Vetta 是 Vetta 的开源版：一个为真实工作而生的 AI Agent。它能读写本地文件、执行命令、
批量处理目录、定时运行任务并安装扩展，把想法、资料和目标转化为可交付的结果，而不只是在对话中给出答案。

它运行在你选择的环境中，可连接你自己的模型、工具与数据。桌面应用、编码智能体内核、CLI、SDK
和扩展点全部开放，让每个人和团队都能围绕自己的工作方式塑造 Agent。

我们相信，AI 的工作方式不应由少数人单独定义。Open Vetta 希望与开发者、创作者和真实用户一起成长：
任何人都可以理解它、扩展它，并共同决定它的下一步。
