...
- **Claude Code Hook profile `claude-code-hooks/2.1.211`**：独立 wire contract（不污染 Codex profile），支持 Vetta 宿主已触发的 10 个事件子集、`command` sync handler、matcher、`${CLAUDE_PLUGIN_ROOT}` / `${CLAUDE_PROJECT_DIR}` 展开、Windows 上禁止将 `.sh` 交给 cmd.exe；配置源为 settings 布局或显式 `hooks/hooks.json` + `CLAUDE_PLUGIN_ROOT`。
