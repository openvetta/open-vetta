...
export interface RetryableCloseControllerOptions {
	readonly cleanup: () => Promise<void>;
	readonly onCompleted?: () => Promise<void> | void;
}

/** Shares one cleanup attempt, retries after failure and finalizes ownership exactly once. */
export class RetryableCloseController {
	private attempt: Promise<void> | undefined;
	private completed = false;

	constructor(private readonly options: RetryableCloseControllerOptions) {}

	run(): Promise<void> {
		if (this.completed) return Promise.resolve();
		if (this.attempt) return this.attempt;

		const operation = this.options.cleanup().then(async () => {
