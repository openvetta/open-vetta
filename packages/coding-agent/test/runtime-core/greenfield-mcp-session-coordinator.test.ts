...
describe("Greenfield MCP session coordinator", () => {
	it("merges shared and plugin MCP state for Session controllers and Subagent inheritance", async () => {
		...
		const resources = new GreenfieldCompositionResourceRegistry();
		const coordinator = await createGreenfieldMcpSessionCoordinator({
			source,
			registry,
