...
import type { ExtensionAPI, SessionBeforeCompactEvent, SessionCompactEvent } from "../src/extensions/index.js";

describe("Documentation example", () => {
	it("custom compaction example should type-check correctly", () => {
		// This is the example from extensions.md - verify it compiles
		const exampleExtension = (api: ExtensionAPI) => {
			api.on("session_before_compact", async (event: SessionBeforeCompactEvent, ctx) => {
	...
	it("compact event should have correct fields", () => {
		const checkCompactEvent = (api: ExtensionAPI) => {
			api.on("session_compact", async (event: SessionCompactEvent) => {
