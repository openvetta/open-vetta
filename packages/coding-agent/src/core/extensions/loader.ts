...
function getOptionalAliases(): Record<string, string> | undefined {
	if (isBunBinary) return undefined;
	try {
		return getAliases();
	} catch {
		// Standalone bundles already carry the public extension modules below.
		return undefined;
	}
}
...
async function loadExtensionModule(extensionPath: string) {
	const aliases = getOptionalAliases();
	const jiti = createJiti(import.meta.url, {
		moduleCache: false,
		// Public extension modules are always injected from this runtime. This covers both
		// compiled binaries and standalone bundles without a sibling node_modules tree.
