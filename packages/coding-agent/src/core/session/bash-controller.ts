...
export class BashController {
	...
	private _bashExecutionPromise: Promise<BashResult> | undefined;
	...
	async executeBash(
		...
		const execution = this.runBash(command, onChunk, options);
		this._bashExecutionPromise = execution;
		try {
			return await execution;
		} finally {
			if (this._bashExecutionPromise === execution) this._bashExecutionPromise = undefined;
		}
	}
	...
	private async runBash(
		...
		this._bashAbortController = controller;
		...
		try {
			...
			if (this._bashAbortController === controller) this._bashAbortController = undefined;
		}
	}
	...
	async quiesceSessionIdentity(): Promise<void> {
		this.abortBash();
		const execution = this._bashExecutionPromise;
		if (execution) await Promise.allSettled([execution]);
		// A result completed while the Agent was streaming belongs to the outgoing
		// transcript. Persist it before SessionManager changes identity.
		this.flushPending();
	}
