...
/**
 * 某个工作目录对应的全局会话分片目录（`<agentDir>/sessions/--<编码后的 cwd>--`）。
 *
 * 纯路径计算，**不建目录**——供只需要知道落点的调用方（会话目录发现、清理、诊断）
 * 使用，避免枚举全部项目时顺手在磁盘上撒一堆空目录。
 */
export function codingAgentSessionShardPath(cwd: string): string {
...
export function resolveCodingAgentSessionDir(cwd: string, sessionDir?: string): string {
	...
	const resolved = codingAgentSessionShardPath(cwd);
