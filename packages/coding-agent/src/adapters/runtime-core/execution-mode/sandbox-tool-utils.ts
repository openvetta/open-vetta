...
import type { SandboxHostServices } from "./sandbox-host-services.js";
...
export function createSandboxToolRegistrations(options: SandboxRuntimeToolOptions): readonly CodingToolRegistration[] {
	const executor = createForegroundCommandToolExecutor({
		operations: options.hostServices.commandOperations,
	...
	const commandRegistration =
		options.hostServices.platform === "win32"
			? createShellToolRegistration(options.cwd, { executor, platform: options.hostServices.platform })
			: createBashToolRegistration(options.cwd, { executor, platform: options.hostServices.platform });
...
export function wrapWorkspaceGuard<TInput extends object>(
	...
	return {
		...
		async execute(request) {
			...
			if (requestedPath) {
				const access = await options.hostServices.resolveWorkspacePathAccess(requestedPath, options.cwd);
				options.hostServices.assertPathNotDenied(access.targetBoundary, tool.name);
				options.hostServices.assertPathNotDenied(access.targetPath, tool.name);
				if (!access.allowed) {
					...
					const cached = sessionId
						? options.hostServices.findSessionGrant(sessionId, permissionRequest)
						: undefined;
					if (!cached) {
						const decision = await confirmSandboxPermission(
							createPermissionContext(options.hostInteraction, request.signal),
							permissionRequest,
							options.hostServices.isDeniedPath,
						);
						if (decision === "deny") {
							const currentAccess = await options.hostServices.resolveWorkspacePathAccess(
								requestedPath,
								options.cwd,
							);
							...
							sessionId &&
							!isSensitiveSandboxRequest(permissionRequest, options.hostServices.isDeniedPath)
						) {
							options.hostServices.addSessionGrant(sessionId, permissionRequest);
						}
					}
				}
			}
	...
	return {
		...
		async execute(request) {
			...
			const permissionRequests = command
				? options.hostServices.collectShellWritePermissionRequests(command, options.cwd)
				: [];
			const sessionId = options.getSessionId?.();
			const allowWriteRoots: string[] = [];
			for (const permissionRequest of permissionRequests) {
				const cached = sessionId ? options.hostServices.findSessionGrant(sessionId, permissionRequest) : undefined;
				if (cached) {
			...
			return options.hostServices.runWithShellGrant(
				options.cwd,
				{
