# Package-root SDK compatibility reference

> New integrations should use the stable [`@vetta/coding-agent/sdk`](stable-sdk.md) contract. This page documents
> the retained package-root `createAgentSession()` API for consumers that still inject concrete host managers or
> arbitrary resource override callbacks.
...
```bash
npm install @vetta/coding-agent
```
...
```typescript
import { createAgentSession } from "@vetta/coding-agent";
...
The `Agent` class (from `@vetta/agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@vetta/coding-agent";
...
import { createAgentSession, type ToolDefinition } from "@vetta/coding-agent";
...
```typescript
import { createEventBus, DefaultResourceLoader } from "@vetta/coding-agent";
...
```typescript
import { createAgentSession, SessionManager } from "@vetta/coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@vetta/coding-agent";
...
```typescript
import { createAgentSession, InteractiveMode } from "@vetta/coding-agent";
...
```typescript
import { createAgentSession, runPrintMode } from "@vetta/coding-agent";
...
```typescript
import { createAgentSession, runRpcMode } from "@vetta/coding-agent";
