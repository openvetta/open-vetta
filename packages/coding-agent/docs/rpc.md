...
**Note for Node.js/TypeScript users**: If you're building a Node.js application, consider using `AgentSession` directly from `@vetta/coding-agent` instead of spawning a subprocess. See [`src/core/agent-session.ts`](../src/core/agent-session.ts) for the API. For a subprocess-based TypeScript client, see [`src/modes/rpc/rpc-client.ts`](../src/modes/rpc/rpc-client.ts).
...
- [`src/model-context/message-types.ts`](../src/model-context/message-types.ts) - `BashExecutionMessage`
...
Extension UI forwarding is implemented by `src/modes/rpc/rpc-extension-ui-bridge.ts` (method names such as `setStatus`, `setHeader`, `setFooter`, `confirm`, `select`).
