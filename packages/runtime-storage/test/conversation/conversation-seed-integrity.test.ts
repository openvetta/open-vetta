...
describe("conversation seed integrity", () => {
	it.each(["import", "continuation"] as const)("accepts valid %s seeds with product graph semantics", (seedKind) => {
	...
	it.each(["import", "continuation"] as const)("maps invalid %s seed graphs to CORRUPT", (seedKind) => {
	...
	it.each(["import", "continuation"] as const)("rejects a missing %s active leaf as CORRUPT", (seedKind) => {
		expect(() =>
...
function sessionId(seedKind: "import" | "continuation"): string {
