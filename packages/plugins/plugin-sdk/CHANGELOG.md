...
- `ctx.media` 宿主媒体消费协议 v2：插件用 `media.generate` 列出 Provider、创建/查询/取消图片或视频任务。输入素材使用插件 Blob 或工作区文件句柄，生成结果使用宿主临时产物句柄，并通过 `saveArtifact()` / `releaseArtifact()` 明确持久化与释放；媒体字节不再经过插件渲染进程。Provider SPI 属于 desktop 主进程，不向插件开放注册。Desktop 内置的 Vetta 图片 Provider 固定在主进程调用网关，插件拿不到 JWT，也不能传任意网关路径（ADR-0057）。
