...
- Agent：插件启用后固定贡献一段内容工作流路由提示、内容创作 Skill，以及 `content_creation_search` / `content_creation_execute` 两个模型工具。`search` 默认只返回紧凑操作索引，按 query 或精确 operation ID 渐进披露当前步骤所需 Schema；`execute` 使用轻量 envelope 路由到 `inspect`、`assets`、`edit`、`run` 领域操作，并在插件边界再次按完整 Schema 校验嵌套输入。路由提示只声明复杂、规范、可审查的工作流适用边界；简单、一次性、要求立即出结果的任务仍走直接能力。素材导入无需确认，编辑批次直接原子执行，生成计划只有在用户点击全局确认弹窗后才会执行。
...
当前可用模型能力、语义连接、连通分量、孤立节点、可运行/阻塞节点和可执行诊断。通过 `content_creation_execute`
执行 `edit` 时携带 `expectedRevision` 原子提交完整批次。每类 operation 使用独立的判别式 Schema，并仅在搜索命中后进入模型上下文；普通拓扑边统一使用
...
本地文件或目录必须先经 `content_creation_execute(operation="assets")` 发现并导入；工作流只保存受管 `blobId`，不保存外部绝对路径。目录默认要求 Agent 明确选择一个文件，批量场景才显式使用 `directoryMode="all"`。
