...
- Agent Skill：内置创意概念、工作流操作、图片创作、视频创作、内容质量审查和多资产 Campaign 六个 Skill；运行时依据最近的用户意图确定性加载对应 Skill 正文与必要参考资料，不再依赖模型主动发现。Agent 编写视频提示词时提交结构化 `promptPlan`，插件统一编译并对实际生效的下游提示词执行方法完整性检查；缺失引用角色、运动、运镜动机、灯光行为或终帧等信息时，整批编辑保持不变并返回可修复诊断。
