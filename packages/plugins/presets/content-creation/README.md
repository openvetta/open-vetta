...
- Agent：插件启用后以固定注册序贡献内容创作 Skill 与 `inspect`、`assets`、`edit`、`run` 四个领域工具，不注册静态或动态 System Prompt；输入栏不再提供“内容创作”开关，用户措辞不会逐轮增删模型工具 Schema 或改写 system 前缀。`assets` 将宿主已授权路径中的本地媒体复制进受管素材存储并返回稳定节点/素材 ID，导入无需确认；`edit` 在会话内首调经 heavy 工具闸确认，后续创建、编辑、删除与连线不再逐操作预览确认；生成计划只有在用户点击全局确认弹窗后才会执行。
- Agent Skill：内置创意概念、工作流操作、图片创作、视频创作、内容质量审查和多资产 Campaign 六个 Skill；模型根据稳定 Skill 索引调用宿主 `invoke_skill`，Skill 正文作为工具结果进入消息历史，再按任务读取文生视频、单图动画、首尾帧、全能参考或视频转换资料，而不是由插件按关键词注入 system prompt。高层 `configure_video_shot` 使用五种判别式 `promptPlan` 合同表达各自的世界构建、源图继承、端点过渡、多参考关系或视频变换语义，并拒绝计划类型与实际策略不一致；首尾帧分别使用静态 `image-keyframe` 计划，首帧保留自身已有的合法素材依据，尾帧以首帧产物为唯一视觉参考进行图生图，再强制交给具备 `firstFrame` / `lastFrame` 输入的插值视频模式。
