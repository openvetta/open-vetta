...
- Agent Skill：内置创意概念、工作流操作、图片创作、视频创作、内容质量审查和多资产 Campaign 六个 Skill；视频创作进一步按文生视频、单图动画、首尾帧、全能参考和视频转换拆分方法资料，运行时依据最近的用户意图只加载共享导演基础与命中的策略方法，不再把五套知识固定塞进同一上下文。高层 `configure_video_shot` 使用五种判别式 `promptPlan` 合同表达各自的世界构建、源图继承、端点过渡、多参考关系或视频变换语义，并拒绝计划类型与实际策略不一致；首尾帧分别使用静态 `image-keyframe` 计划，首帧保留自身已有的合法素材依据，尾帧以首帧产物为唯一视觉参考进行图生图，再强制交给具备 `firstFrame` / `lastFrame` 输入的插值视频模式。
