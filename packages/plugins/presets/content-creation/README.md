...
- Agent Skill：内置创意概念、工作流操作、图片创作、视频创作、内容质量审查和多资产 Campaign 六个 Skill；视频创作进一步按文生视频、单图动画、首尾帧、全能参考和视频转换拆分方法资料，运行时依据最近的用户意图只加载共享导演基础与命中的策略方法，不再把五套知识固定塞进同一上下文。高层 `configure_video_shot` 使用五种判别式 `promptPlan` 合同表达各自的世界构建、源图继承、端点过渡、多参考关系或视频变换语义，并拒绝计划类型与实际策略不一致；首尾帧仍分别使用静态 `image-keyframe` 计划。
...
`inspect` 同时返回每个视频节点当前实际采用的生成策略、对应 `promptPlan` 类型、方法说明、输入合同与来源角色，并诊断通用提示词缺少策略方法、首尾帧复用提示词、关键帧误用视频提示词、全能参考缺少语义清单以及已连接 Prompt 被本地文本遮蔽等问题。高层视频配置会把动态 Prompt 节点内容与分策略编译后的导演计划显式组合，保证画布连接、创作方法和实际请求一致。
