...
- 插件配置由插件自己的工作区配置页承载（ADR-0105）：API Key 走 `ctx.secrets`（宿主 `CredentialVault`），模型与 baseUrl 等普通字段走 `ctx.storage`，宿主设置页不再有插件配置槽。
...
1. 插件的工作区配置页收集 API Key，调用 `ctx.secrets.set(key, value)`（需要 `secrets.write`）。
2. 主进程按 capability session 反查 plugin id，写入 Electron `safeStorage` 支持的统一凭据库；插件不接触凭据文件和加密实现。
3. `ContentSettingsStore` 在激活时把普通字段（`ctx.storage` 的 JSON key `settings`）与密钥合成一份同步视图，供各 Provider 构造请求时同步取值。
4. 配置页只显示密钥的「已保存 / 未配置」状态，不回显已保存的值；写入空串等价于删除。
5. 旧版本 `plugin-settings.json` 里的普通字段由宿主一次性迁入插件私有存储；密钥的凭据库命名空间未变，无需迁移。

安全存储不可用时，密钥写入会失败，不会退化为明文保存。
...
用户选择或拖入的真实文件通过 `PluginStorageApi.putBlobFromFile()` 交给宿主直接复制，不经过 renderer Base64；内存生成的数据仍可使用 `putBlob()`。项目文档只保存稳定的 `assetId` / `blobId`、素材元数据与槽位绑定，不保存宿主生成的媒体 URL。界面加载项目后通过 `getBlobRef(blobId)` 解析临时预览 URL；宿主媒体 Provider 直接消费 `plugin-blob` / `workspace-file` 句柄，只有要求 inline data 的旧 Provider Adapter 才在请求阶段按需读取内容。供应商字段不会进入项目 schema。
