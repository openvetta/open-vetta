...
export function renderContentMethodContext(methodIds: readonly ContentMethodId[]): string {
	...
	return [
		...
		"Prompt nodes are optional reuse primitives. Keep single-use prompts on their generator; add a Prompt node only for a verbatim fragment intentionally shared by at least two consumers or explicitly requested central reuse.",
		...
		"Image keyframe plans describe frozen visible states; video plans describe continuous state change. Never reuse one prompt across those node kinds. For first/last-frame, configure_video_shot must preserve the first generator's intentional authorities, edit last from the generated first image, and bind both to one interpolation-capable video mode.",
