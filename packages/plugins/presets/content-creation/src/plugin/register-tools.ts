...
export function registerContentCreationTools(
	...
	ctx.agent.registerTool<EditInput>({
		...
		description:
			"Atomically apply a revision-bound batch of typed workflow operations. The first call in a session asks the user to confirm, because this writes into their workspace. Each operation has its own required fields. Ordinary connections use sourceNodeId/targetNodeId/optional edgeId. Prefer configure_video_shot for Agent-authored video work; declare video media only in its sources or keyframes, never as duplicate connect_nodes operations. Choose its strategy-specific prompt plan for text, single-frame, first/last-frame, omni-reference, or transform work; automatic resolves declared authorities but does not replace method selection. The operation composes connected Prompt nodes, validates plan/strategy agreement, and configures roles without silent degradation. Keep configure_generation for low-level compatibility or role repair. The returned readiness analysis identifies incomplete graphs.",
		parameters: {
			type: "object",
			properties: {
				projectDir: projectDirProperty,
