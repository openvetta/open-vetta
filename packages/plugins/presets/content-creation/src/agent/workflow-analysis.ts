...
export function analyzeContentWorkflow(
	...
	const issues = [...baseIssues, ...diagnoseGraph(project)];
...
function diagnoseGraph(project: ContentProjectDocument): ContentWorkflowAnalysisIssue[] {
	...
	return models.some(
		(model) =>
			...
			(!node.data.providerId ||
				!node.data.modelId ||
				(model.providerId === node.data.providerId && model.modelId === node.data.modelId)),
	);
}
...
function semanticConnection(project: ContentProjectDocument, edge: ContentEdge): ContentSemanticConnection | null {
	...
	const toInput = semanticInput(target, edge.targetHandle);
...
function semanticInput(
	...
	if (node.kind === "video-generator" && targetHandle === "image") return "startImages";
	if (node.kind === "video-generator" && targetHandle === "video") return "referenceVideos";
