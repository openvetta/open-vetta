...
function diagnoseGenerator(
	...
	if (!prompt) {
		...
		diagnoseFirstLastFrameTopology(project, node, diagnostics);
	}
...
function diagnoseFirstLastFrameTopology(
	project: ContentProjectDocument,
	videoNode: ContentNode,
	diagnostics: ContentAgentDiagnostic[],
): void {
	const firstFrameEdges = project.graph.edges.filter(
		(edge) => edge.target === videoNode.id && edge.role === "firstFrame",
	);
	const lastFrameEdges = project.graph.edges.filter(
		(edge) => edge.target === videoNode.id && edge.role === "lastFrame",
	);
	if (firstFrameEdges.length !== 1 || lastFrameEdges.length !== 1) return;
	const firstFrameNode = project.graph.nodes.find((node) => node.id === firstFrameEdges[0]?.source);
	const lastFrameNode = project.graph.nodes.find((node) => node.id === lastFrameEdges[0]?.source);
	if (firstFrameNode?.kind !== "image-generator" || lastFrameNode?.kind !== "image-generator") return;
	const derivesLastFromFirst = project.graph.edges.some(
		(edge) =>
			edge.source === firstFrameNode.id &&
			edge.target === lastFrameNode.id &&
			edge.targetHandle === "reference" &&
			edge.role === "referenceImages",
	);
	if (!derivesLastFromFirst) {
		diagnostics.push({
			code: "video-keyframe-derivation-missing",
			severity: "error",
			nodeId: videoNode.id,
			message: "The last frame must be generated by editing the first-frame image, not as an independent still.",
			retryable: true,
			details: {
				firstFrameNodeId: firstFrameNode.id,
				lastFrameNodeId: lastFrameNode.id,
				requiredDependency: `${firstFrameNode.id} -> ${lastFrameNode.id}`,
				recommendedOperation: "configure_video_shot",
			},
		});
	}
	if (
		lastFrameNode.data.modeId !== "image-to-image" ||
		videoNode.data.modeId !== "image-to-video"
	) {
		diagnostics.push({
			code: "video-keyframe-modes-invalid",
			severity: "error",
			nodeId: videoNode.id,
			message: "First/last-frame video requires an image-to-image last frame derived from the first frame and image-to-video interpolation.",
			retryable: true,
			details: {
				lastFrameModeId: lastFrameNode.data.modeId ?? null,
				videoModeId: videoNode.data.modeId ?? null,
				recommendedOperation: "configure_video_shot",
			},
		});
	}
