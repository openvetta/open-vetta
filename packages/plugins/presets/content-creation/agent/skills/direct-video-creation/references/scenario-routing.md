...
| Character story or music video | `narrative-action-and-tutorial-video-recipes.md` | authority ledger -> ordered scene/keyframe branches -> timestamped prompts |
...
| Long video to Shorts/Reels/TikTok | `camera-social-and-clipping-video-recipes.md` | transcript/candidates -> rank/dedupe -> source ranges/crops -> audit |

## Selection rules

- A named visual effect does not replace a shot plan. Define event, camera, sound, and final state.
- A multi-shot request becomes separate ordered shot nodes unless inspected capabilities explicitly support timestamped multi-shot direction inside one generation.
