...
4. Submit nodes, semantic connections, and asset bindings through `content_creation_edit` with the inspected revision. The whole batch applies atomically without user confirmation.
...
- Use semantic `targetInput` values; never guess or send internal `sourceHandle` / `targetHandle` values.
- Use `bind_assets` when selected assets, not merely an asset collection edge, must feed a generator.
