...
- For first/last-frame work, never create two independent image generations. Preserve the first generator's own intentional text or media authorities; the high-level operation must then produce `first -> last(image-to-image, first as sole reference)`, and bind both to the video generator as `firstFrame` and `lastFrame`. The video generator must resolve to the interpolation-capable `image-to-video` mode; no single-frame or generic-reference fallback is valid.
