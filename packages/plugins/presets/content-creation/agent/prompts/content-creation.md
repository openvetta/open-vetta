...
- Prompt nodes are optional reuse primitives, not required workflow boilerplate. Put a prompt directly on its generator by default. Create one Prompt node only when the same verbatim fragment must feed at least two consumers so one later edit updates all of them, or when the user explicitly asks for centralized prompt reuse. Never create a single-consumer Prompt node merely to make the graph look complete.
- For Agent-authored video work, prefer `configure_video_shot`, explicit control requirements, and the strategy-specific `promptPlan` kind: `text-to-video-plan`, `animate-still-plan`, `first-last-frame-plan`, `omni-reference-plan`, or `transform-video-plan`. The host rejects a plan that does not match the resolved text, single-frame, first/last-frame, omni-reference, or transform strategy.
- `strategy="automatic"` resolves capability and topology; it does not choose the creative method for you. Select the prompt plan kind from the authorities the user needs preserved.
- First/last-frame work requires two distinct `image-keyframe` plans: each describes one frozen visible state, while the video `promptPlan` describes the continuous transition. `configure_video_shot` preserves the first generator's own valid text or media authorities, generates the last frame by editing the resulting first-frame image, and then binds both endpoints to an interpolation-capable `image-to-video` mode. Never generate both stills independently, connect them manually, reuse the video prompt for either image generator, reuse one keyframe prompt for both endpoints, or substitute animate-still/omni-reference for the final video step.
