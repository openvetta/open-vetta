...
describe("parseRemoteCatalog", () => {
	it("解析合法清单并带出出处与标语", () => {
		const result = parseRemoteCatalog(catalog([entry()]));
	...
	it.each([
		...
		expect(parseRemoteCatalog(raw)).toBeNull();
	});
	...
	it.each([
		["spec 自带 frontmatter（会写出两段）", { content: { spec: "---\nsystem: x\n---\n正文", theme: "@theme {}" } }],
		["theme 不是 @theme 块", { content: { spec: "# ok", theme: "body { color: red }" } }],
		["spec 为空", { content: { spec: "   ", theme: "@theme {}" } }],
		["缺 content", { content: undefined }],
		["slug 形态非法", { slug: "../escape" }],
		["vibe 不合法", { vibe: "neon" }],
		["缺 blurb", { blurb: "" }],
	])("丢弃不合格条目：%s", (_label, overrides) => {
		const result = parseRemoteCatalog(catalog([entry(), entry({ slug: "bad", ...overrides })]));
		expect(result?.systems.map((system) => system.id)).toEqual(["linear"]);
		expect(result?.rejected).toHaveLength(1);
	});

	it("同 id 只保留第一条", () => {
		const result = parseRemoteCatalog(catalog([entry(), entry({ name: "Linear Dup" })]));
		expect(result?.systems).toHaveLength(1);
	...
	it("超长正文按畸形源丢弃", () => {
		const huge = `# x\n${"a".repeat(64 * 1024)}`;
		const result = parseRemoteCatalog(catalog([entry(), entry({ slug: "huge", content: { spec: huge, theme: "@theme {}" } })]));
		expect(result?.systems.map((system) => system.id)).toEqual(["linear"]);
	});

	it("没有 tagline 时不编造，留给 i18n 回落", () => {
		const result = parseRemoteCatalog(catalog([entry({ tagline: { en: "only-en" } })]));
