...
/**
 * 内置的设计体系：随插件打包，作为远端清单拉不到时的兜底（离线、首屏、
 * 网络受限都靠它）。远端清单可用时由 registry 整体替换。
 *
 * 顺序即 drawer 里的展示顺序：按风格差异交错排（亮暗、冷暖、克制/张扬），
 * 让用户横滑前几张就能碰到完全不同的方向。远端清单用 meta.json 的 `order` 表达同一件事。
 *
 * token 契约：每套 theme.css 必含 DEFAULT_THEME_CSS 的 7 个基础 color token
 * （只换值不改名），并统一追加 surface-raised / border 与 radius / shadow 刻度。
 */
export const BUILTIN_DESIGN_SYSTEMS: readonly DesignSystem[] = [
