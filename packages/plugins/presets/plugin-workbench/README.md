...
- **Desktop 构建会自动同步**：`apps/desktop-app` 的 `bun run build:presets` / `build:presets:dev`（dev 与 dist 都走）在构建前调用 `scripts/sync-plugin-docs.mjs`，把 monorepo `docs/plugin` 拷进本包；一般**不必**再手跑 sync。
