...
export async function saveCowartCanvasSnapshot(args = {}, snapshot) {
  ...
  if (process.env.COWART_VETTA === "1" || process.env.COWART_DISABLE_WIDGET === "1") {
    sanitized = { snapshot, skippedRecords: [] };
  } else {
    const { sanitizeCanvasSnapshotForTldraw } = await import("./canvasSnapshot.js");
    sanitized = sanitizeCanvasSnapshotForTldraw(snapshot);
  }
