...
Vite 需要暴露 `./theme`，生成 `mf-manifest.json`，并将 React、`@vetta/theme-sdk` 与宿主 UI 包配置为不重复打包的共享单例。构建后把主题接入桌面应用的策展清单，通过根目录 `bun run verify:ui:*` 流程检查加载、切换、明暗模式、窗口尺寸和主题页面。当前没有独立 ZIP 安装步骤。
