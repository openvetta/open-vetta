...
function getEndpointFilePath(userDataPath: string): string {
	return join(userDataPath, "action-server.json");
}
...
export async function startLocalActionServer(
	...
	const endpointFilePath = getEndpointFilePath(options.userDataPath);
