...
function getFallbackThemeHelp(): Record<string, JsonValue> {
	return {
		...
		operations: [
			{
				type: "help",
				description: "返回可用操作、mode / cursorStyle 取值和当前 renderer 已注册的 themeId 列表。",
				input: { type: "help" },
			},
			{
				type: "get",
				description: "读取当前显示模式、主题风格、鼠标指针、解析后的亮暗模式和原生主题信息。",
				input: { type: "get" },
			},
			{
				type: "set",
				description:
					"设置外观。agent 已明确 mode / themeId / cursorStyle 时直接传值；需要让用户查看并选择时，使用 approvalUi: appearance.picker，且可省略全部字段。",
				input: { type: "set", mode: "dark", themeId: "default", cursorStyle: "default" },
		...
		cursorStyles: [
			{ value: "default", description: "系统默认鼠标指针（设置 → 外观 → 鼠标指针）。" },
			{ value: "stoat", description: "白鼬自定义鼠标指针（设置 → 外观 → 鼠标指针）。" },
		],
		approvalUi: {
			parameter: "approvalUi",
			default: "appearance.theme-change",
			guidance:
				"用户未指定具体外观选项，或适合让用户比较并自行决定时，优先使用 appearance.picker；用户已明确目标值时可省略 approvalUi，使用默认确认界面。",
			options: [
				{
					value: "appearance.theme-change",
					interactive: true,
					description:
						"默认的外观变更确认界面。可确认/调整 agent 提供的 mode、themeId、cursorStyle；至少提供其中一项（除非使用 picker）。",
				},
				{
					value: "appearance.picker",
					interactive: true,
					description:
						"可交互外观选择器。用户可查看并调整显示模式、主题风格与鼠标指针；各字段可作为初始值，也可全部省略并使用当前外观作为初始选择。",
				},
				{
					value: "generic",
					interactive: false,
					description:
						"通用 Action 确认界面。展示完整输入但不提供外观选择控件；至少提供 mode / themeId / cursorStyle 之一，仅在需要通用审批展示时使用。",
				},
			],
		},
		themes: [],
