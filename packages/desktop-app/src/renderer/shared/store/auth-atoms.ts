...
// ─── Subscription status (Vetta Go / Zen 套餐，ADR-0016 离线回退) ───
...
/**
 * 套餐状态读写 atom。
 * - 读：返回当前内存状态；若尚未拉取(null)，回退到 localStorage 缓存的 zen/go 标志构造的最小状态。
 * - 写：set SubscriptionStatus 时持久化 zen/go 标志；set null 不动缓存(离线/失败时仍用上次已知)。
 */
export const subscriptionStatusAtom = atom<SubscriptionStatus, [SubscriptionStatus | null], void>(
	(get): SubscriptionStatus => {
		...
		return {
			active: false,
			zen_enabled: cached.zen_enabled,
	...
	(_get, set, next) => {
		...
		if (next) {
			persistFlags({ zen_enabled: next.zen_enabled, go_enabled: next.go_enabled });
		}
	},
);
