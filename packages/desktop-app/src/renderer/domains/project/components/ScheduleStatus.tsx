		...
		<div className="flex items-center justify-between rounded-xl border border-border/30 bg-muted/15 px-4 py-3 transition-colors duration-200 hover:bg-muted/25">
					...
					<div className="mt-0.5 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-[11px] text-muted-foreground/50">
						...
						<span className="flex items-center gap-1">
							...
							{task.lastRunStatus === "success" && (
								<span className="text-emerald-500">{t("list.success")}</span>
							)}
							{task.lastRunStatus === "failed" && <span className="text-red-400">{t("list.failed")}</span>}
						</span>
					</div>
				</div>
			</div>
			<div className="flex shrink-0 items-center gap-1.5">
				...
				>
					...
					<span className="text-[12px]">{t("list.run")}</span>
				</Button>
			</div>
		</div>
	);
	...
	return (
		<div className="px-8 py-5">
			<div className="mb-3 flex items-center gap-2.5">
				...
				<h2 className="text-[13px] font-semibold text-foreground">{t("list.sectionTitle")}</h2>
			</div>
