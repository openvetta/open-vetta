...
export function NotificationMessageListView(model: NotificationMessageListModel): JSX.Element {
	...
	return (
		<div className="flex flex-col gap-1.5 p-3">
			{(model.hasUnread || model.hasRead) && (
				<div className="flex justify-end gap-1.5 px-0.5">
					{model.hasUnread && (
						<MessageCenterToolbarButton icon="icon-[solar--check-read-linear]" onClick={model.onMarkAllRead}>
							{model.markAllReadLabel}
						</MessageCenterToolbarButton>
					)}
					{model.hasRead && (
						<MessageCenterToolbarButton
							icon="icon-[solar--notification-lines-remove-linear]"
							onClick={model.onClearRead}
						>
							{model.clearReadLabel}
						</MessageCenterToolbarButton>
					)}
				</div>
			)}
			<AnimatePresence initial={false} mode="popLayout">
				{model.items.map((notification) => (
					<motion.div
						...
						onClick={() => model.onMarkRead(notification.id)}
						className={cn(
					...
					>
						<button
							...
							onClick={(event) => {
								...
								model.onDelete(notification.id);
							}}
