	...
	return (
		<div className="mx-auto w-full max-w-2xl space-y-6 px-8 pb-12">
			...
			<div className="flex items-center justify-between">
				<div>
					<SettingHeading t={t as any} section={SETTINGS_SECTION["team-management"]} className="text-[18px] font-bold" />
					<p className="mt-1 text-[12px] text-muted-foreground">{t("teamCreateOrJoin", { members: t("teamMembers") })}</p>
				</div>
...
function TeamList({
	...
	if (teams.length === 0) {
		return (
			<SettingSection t={t as any} section={SETTINGS_SECTION["team-my-teams"]}>
				<div className="px-5 py-8 text-center text-[13px] text-muted-foreground">
					{t("teamJoinViaCode", { codeLabel: t("teamCodeLabel") })}
				</div>
			</SettingSection>
		);
	}
					...
					<div className="flex items-center gap-3">
						...
						<div>
							...
							<div className="text-[11px] text-muted-foreground">
								{team.role === "owner" ? t("teamOwner") : team.role === "admin" ? t("teamAdmin") : t("teamMember")}
							</div>
						</div>
					</div>
...
function TeamDetail({
	...
	return (
		<div className="space-y-4">
			...
			{/* Team info */}
			<SettingSection t={t as any} section={SETTINGS_SECTION["team-detail-info"]} title={detail.name}>
				{/* Invite code */}
				<div className="flex items-center justify-between border-b border-border px-5 py-3.5">
					<div>
						<div className="text-[13px] font-medium text-foreground">{t("teamCodeLabel")}</div>
					...
					<div className="flex gap-1.5">
						...
						<Button variant="ghost" size="xs" onClick={handleResetCode} title={t("teamResetCode")}>
				...
				<div className="flex items-center justify-between px-5 py-3.5">
					<div className="text-[13px] font-medium text-foreground">{t("teamMembers")}</div>
					<div className="text-[12px] text-muted-foreground">{t("teamMemberCount", { count: detail.members.length })}</div>
				</div>
			</SettingSection>

			{/* Members */}
			<SettingSection t={t as any} section={SETTINGS_SECTION["team-members"]}>
						...
						<div className="flex items-center gap-3">
							...
							<div>
								...
								<span className="ml-2 text-[11px] text-muted-foreground">
									{member.role === "owner" ? t("teamOwner") : member.role === "admin" ? t("teamAdmin") : t("teamMember")}
								</span>
							</div>
						</div>
						{member.role !== "owner" && (
...
function CreateTeamDialog({
	...
	const handleCreate = async () => {
		...
		try {
			...
			setError(e instanceof Error ? e.message : t("teamCreateFailed"));
		} finally {
			setLoading(false);
		}
	};
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent>
				<DialogHeader>
					<DialogTitle>{t("teamCreate")}</DialogTitle>
				</DialogHeader>
				...
				<DialogFooter>
					...
					<Button onClick={handleCreate} disabled={loading || !name.trim()}>
						{loading ? t("teamCreating") : t("teamCreateBtn")}
					</Button>
				</DialogFooter>
			</DialogContent>
		</Dialog>
...
function JoinTeamDialog({
	...
	const handleJoin = async () => {
		...
		try {
			...
			setError(e instanceof Error ? e.message : t("teamJoinFailed"));
		} finally {
			setLoading(false);
		}
	};
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent>
				<DialogHeader>
					<DialogTitle>{t("teamJoin")}</DialogTitle>
				</DialogHeader>
				<div className="space-y-3">
					<Input
				...
				<DialogFooter>
					...
					<Button onClick={handleJoin} disabled={loading || !code.trim()}>
						{loading ? t("teamJoining") : t("teamJoinBtn")}
					</Button>
				</DialogFooter>
			</DialogContent>
		</Dialog>
