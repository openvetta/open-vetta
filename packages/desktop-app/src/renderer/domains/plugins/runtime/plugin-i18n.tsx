 ...
 */
export function usePluginTextResolver(): (pluginId: string, raw: string | undefined) => string {
	const locale = useAtomValue(languageAtom);
	...
	return useCallback(
		(pluginId, raw) => {
			...
			return resolvePluginText(raw, entry?.locales ?? {}, locale, entry?.defaultLocale ?? "zh");
		},
		[locale, registry],
	);
}
