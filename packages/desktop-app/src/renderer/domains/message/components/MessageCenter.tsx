...
function FlowingList(): JSX.Element {
	...
	if (pendingList.length === 0) {
		return <EmptyState text={t("empty.flowing")} icon="icon-[solar--transfer-horizontal-linear]" />;
	}

	return (
		<div className="flex flex-col gap-1.5 p-3">
			<AnimatePresence initial={false}>
				{pendingList.map((tx, i) => (
					<motion.div
						key={tx.id}
						layout
...
function NotificationList(): JSX.Element {
	...
	if (notifications.length === 0) {
		return <EmptyState text={t("empty.notifications")} icon="icon-[solar--bell-linear]" />;
	}
...
function ChatList({ onClose }: { onClose: () => void }): JSX.Element {
	...
	if (items.length === 0) {
		return <EmptyState text={t("empty.chat")} icon="icon-[solar--chat-round-line-linear]" />;
	}
						...
						<div className="min-w-0 flex-1">
							<div className="flex items-center justify-between gap-2">
								<p className="truncate text-[12.5px] font-semibold text-foreground">
									{s.project_name || t("chat.flowingFallback", { id: s.flowing_id })}
								</p>
							...
							<p className="mt-1 truncate text-[11.5px] text-muted-foreground">
								{s.last_sender ? `${s.last_sender}${t("chat.senderSuffix")}` : ""}
								{s.last_content || t("chat.newMessage")}
							</p>
							{s.last_created_at && (
												...
												{activeTab === "all" && (
													<>
														...
														{chatUnread === 0 && pendingCount === 0 && notifUnread === 0 && (
															<EmptyState text={t("empty.all")} icon="icon-[solar--inbox-linear]" />
														)}
													</>
												)}
