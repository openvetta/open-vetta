...
const components: Components = {
	...
	// Lists
	ul: ({ children }) => (
		<ul className="my-1.5 ml-4 list-disc space-y-0.5 text-[13px] leading-[1.6] text-[var(--text-1)]">{children}</ul>
	),
	ol: ({ children }) => (
		<ol className="my-1.5 ml-4 list-decimal space-y-0.5 text-[13px] leading-[1.6] text-[var(--text-1)]">{children}</ol>
	),
	...
	// Links
	a: ({ href, children }) => (
		<a href={href} className="text-[var(--md-link)] underline decoration-[var(--md-link)]/30 hover:decoration-[var(--md-link)]" target="_blank" rel="noopener noreferrer">
			{children}
		</a>
	),
