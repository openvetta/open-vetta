...
- **能力市场来源管理**：能力页「刷新」左侧新增「来源」按钮（GitHub 图标），弹窗内可对 GitHub 市场来源增删改查——添加仓库、改名称/分支、启停、删除。此前添加的外置仓库没有任何入口可以删除。内置的官方来源「Vetta Official」（`https://github.com/openvetta/vetta-official-marketplace`）默认存在、可启停但不可删除或改配置；未设置 `VETTA_OPEN_MARKETPLACE_REPOSITORY` 时也会自动创建它。「添加能力」下拉里的「添加外置仓库」入口随之删除，统一收敛到「来源」。
