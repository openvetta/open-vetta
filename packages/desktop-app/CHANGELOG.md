...
- **主进程在 streaming 后偶发返回 `__filename is not defined` 并随后被 OOM Killer 静默 SIGKILL**：根因两条，分层修。(1) `vite.main.config.ts` 把主进程打成 ESM bundle，但只 external 了 `@silvia-odwyer/photon-node`，其它含 native loader 的 CJS 包（`dbus-next` → `usocket` → `bindings`、`@mariozechner/clipboard`）全被内联进 ESM bundle；`bindings/lib/bindings.js:getFileName()` 内部裸用 `__filename` 没 typeof 守护，rollup 不改写这些标识符，运行时一旦执行就抛 `ReferenceError: __filename is not defined`，被 `openai-completions.ts:342` catch 当成 errorMessage 暴露在 UI 上，看着像模型回了这段文本。修法：把上述 CJS 包全部 external，配合 `prepare-pack.js` 的 `asarUnpack` 让其 native `.node` / `.wasm` 文件以真实路径落地，运行时通过 `createRequire` 走 Node 真正的 CJS 加载器（那里 `__filename` 真实存在）。(2) 主进程从未给 V8 设过老生代上限，长跑批量任务 + 不缩放图片喂模型（photon WASM 之前因同一 ESM/CJS 问题 load 失败）后 RSS 自然膨胀，最终被 Linux OOM Killer SIGKILL，进程静默消失、连一行日志都来不及写。`main.ts` 在 app 启动前 `appendSwitch("js-flags", "--max-old-space-size=4096")`，超限时改由 V8 抛 `RangeError: JS heap out of memory`，可被 `uncaughtException` handler 接到并落盘栈；CLI 模式跑短任务沿用默认。
