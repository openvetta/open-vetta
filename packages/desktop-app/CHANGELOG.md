...
- **设计画廊（系统插件「Vetta UI Design」新增侧边栏入口「设计」）**：所有带设计稿的项目的注册中心。它主动收集侧边栏里**项目根目录下**直接躺着 `x.vetd/` 的项目（只 readDir 一层，不递归——画廊要把每个项目都扫一遍，递归大仓库会让这个页面打不开），一个项目一张卡，卡面是**上封面下 info** 的 Figma 式版式：封面为画布全景，info 为项目名 + 最近改动相对时间 + 多设计时的「N 份设计」+ 有会话在跑时的运行中指示。点卡片跳进该项目**最近一个可续聊的会话**（只读或已被别的运行时占用的会话会被跳过，一个都没有则落到该项目的新建会话页），并直接把设计画布铺开、定位到卡面那份设计——从画廊进来是明确的「我要看这份设计」，不该再让用户自己去活动面板找标签卡。工具栏可按名字搜索、新建（只问名字，项目建在 workspace 下并铺一份空设计）、导入（拖 `.vetdz` 到页面，或按钮选文件）；卡片右键可导出分享包（项目里有多份设计时开子菜单让用户选，不替他挑）、在文件管理器中显示、归档项目。画廊只在**工作模式**下出现，与设计画布同属一档能力（ADR-0046）。
- **画布封面**：离开一份设计时（切设计稿、关面板、切会话），用 manifest 里的 frame 坐标把画布已有的逐帧位图拼成一张**原比例全景图**存进 IndexedDB，供画廊卡片显示。素材复用画布本来就截过的那批位图，不额外起引擎、不额外截图，也不往用户项目里写派生文件。因此「有没有封面」等价于「这台机器上有没有开过这份设计的画布」——刚 clone 或刚导入的设计没有封面，卡片改用该设计 `theme.css` 的主色刷一个带设计名的占位块，开过一次画布后下次就有真封面了。全景按原比例保存、卡片内 `object-cover` 裁切，故 frame 横排得很长的设计在卡面上只看得到中间一段。
