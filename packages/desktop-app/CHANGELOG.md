...
- **看板 0.2.2：模型清单补齐 Vetta Go**：插件侧的模型清单此前只来自主进程的本地模型配置，看不到登录后服务端下发的远程目录，于是看板的模型选择器缺了 Vetta Go 一整块。现在 renderer 的 `official.models` 把两份按宿主选择器同一口径合并（同 key 本地优先），模型 key 校验也认远程 key。看板的模型清单改为每次面板挂载刷新一次，开着看板去登录或加 provider，回来即可看到新模型。
