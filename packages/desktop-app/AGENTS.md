...
**新增或改动行为（交互判定、状态机、IPC 契约、纯业务逻辑等）必须附带测试并本地跑通，不能只靠手点。** 纯样式微调且无行为变化可例外。

**选型原则：按被测对象选最轻、最稳的方式，不要默认上整棵 React 树或某种 DOM 环境。**

| 被测对象 | 优先做法 | 本包现状 |
|----------|----------|----------|
| 纯函数 / 策略 / 映射 / 校验 | 抽出独立模块，`*.test.ts` + Vitest（默认 `node`） | 主路径；`src/**/*.test.ts` |
| 需要 DOM API 的渲染交互 | 文件头 `// @vitest-environment jsdom`，可用 `createRoot` + 事件（见 activity-panel 等现有测法） | 依赖已有 `jsdom`，按文件 opt-in |
| 跨窗口 / 打包启动等 | E2E（`bun run test:e2e`） | 冒烟级；**不替代**单测 |
| UI 观感与连调 | `verify:ui:*` | **不是**功能测试，见下文 |

- 优先把「该不该关 / 是否放行 / 如何映射」写成**纯函数**再测（例：`ability-detail-modal-guard`）；theme-ui 侧同理。
- 不为本点功能强行引入尚未统一接入的框架（如 RTL、happy-dom）；若现有手段不够，再论证新增依赖，并与仓库其它测试风格对齐。
- 跑测：包内 `bun run test`，或仓库根 `bunx vitest --run <具体 test 路径>`。改完相关逻辑后至少跑通本次新增/改动的测试。
