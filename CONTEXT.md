...
[[系统通知]] 的判别联合标签。每种类型自带 payload 形状与「点击后做什么」的路由意图。当前成员：[[agent 完成通知]]、[[agent 提问待确认通知]]（payload 均携带定位目标 session 所需的标识），以及两条与 [[手机镜像]] 相关的安全提示：[[手机已连接通知]] 与 [[手机配对请求通知]]（点击路由到「远程连接」设置页）。新增类型 = 加一个联合分支 + 在薄 dispatch 里补一段判定/构造/路由，**不触碰**已有类型。

### 手机已连接通知

[[通知类型]] 成员：一部已配对手机与本机建立链路（局域网或中继）时触发，正文含设备名。**永不抑制**：它的意义就是让用户察觉有设备接入，即便二维码被偷拍也能立刻发现并去「远程连接」解除配对。同一设备名合并为一条。

### 手机配对请求通知

[[通知类型]] 成员：手机通过手动输入 IP 发起配对、等待用户在桌面比对 [[验证码]] 放行时触发，正文含设备名与验证码。永不抑制；同一时刻只保留一条。
...
已配对手机上看到的桌面状态：与桌面窗口共用同一批运行时会话实例（按会话文件路径去重、打开时不改执行模式），覆盖「对话」目录与全部已打开项目，包含电脑上发起的轮次与等待回答的提问；手机可继续追问、代答、中止、新建。会话在协议里用文件路径哈希作不透明 id。镜像只在至少一部手机在线时存在，最后一台断开即卸载——没有手机时桌面端零负担（ADR-0128）。与 [[远程网关]] 无关：那是模型请求的出口，这是手机进入桌面的入口。

### 验证码（pairing verification code）

手动配对时两端各自由双方身份公钥算出的同一个 6 位数字。用户在电脑上比对手机屏幕上的数字后放行，用来排除同网段的中间人；扫码配对不需要它，因为二维码已携带电脑的身份公钥。
