...
全量门禁：`bun run check` 通过（lint 2750 文件、5 个 typecheck 目标、13 项架构守卫）；`apps/desktop-app` 渲染层既有 76 个测试文件 / 332 条用例全部通过，无回归；`bun run build:presets` 正常产出并 staging `kanban@0.1.0`。
