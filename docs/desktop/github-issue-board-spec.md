...
状态和标签的控件形态与项目选择相同：按钮展示当前值，点开后是菜单，不用原生 `<select>`。
...
- 点状态菜单选「失败」后只看到失败行；再选「全部」后全部回来。
- 点标签菜单选某个标签后只看到带该标签的行。
- 筛空时出现「没有符合筛选的任务」，不出现「还没有拉取 Issues」，筛选控件仍在。
...
| `BoardSelect.tsx` | 状态/标签筛选的自定义下拉，形态对齐项目选择 |
