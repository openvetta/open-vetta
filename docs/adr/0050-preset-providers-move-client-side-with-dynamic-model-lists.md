...
**默认每个系列只保留最新一档。** 各家 `/models` 会把历年模型全列出来（OpenAI 38 个、Gemini 22 个），绝大多数是没人再用的历史版本。按目录的 `family` 分组、取 `release_date` 最新的一个，并淘汰整族发布超过一年的（`selectLatestModels`）——不写死任何型号，新模型上线自动顶替旧的。目录里查不到的模型一律保留：那通常是刚发布还没进目录、或该账号专属，宁可多留也不能把用户真正能用的新模型藏掉。设置页有「显示全部模型」开关，落 `settings.json`（不是渲染层 localStorage——后台定时同步也要读它），切换后重新拉取并落盘，让模型选择器与开关一致。

已知取舍：折叠粒度取决于目录怎么划 family。Gemini 的 `gemini-pro` 族里混着 deep-research，pro 档会被同族更新的条目挤掉；这是目录数据的粒度问题，不为它引入本地例外表——需要时开「显示全部模型」。
