...
- **查看（peek）与恢复是两件事**：peek 把旧版本写进工作区但**不提交**，画布照常热重载，于是用户看到的是真实、可交互、可翻页的那一版；退出时用 HEAD 覆盖回去，历史里不留痕迹。「不提交」是它全部风险的来源，因此有三道闸：进入前把未提交的现场落成一个版本（否则它会被旧内容覆盖）、peek 期间 `Stop` hook 跳过这份设计（否则回合结束会把旧版本记成新版本，历史从此说谎）、`.history/peek.json` 标记让崩溃或强退在下次打开设计时自动退回最新版。peek 期间对文件的改动会被丢弃——工作区装的是一份旧版本，在它上面改出来的东西既不属于旧版也不属于新版，所以横幅上明说，而不是事后解释。
