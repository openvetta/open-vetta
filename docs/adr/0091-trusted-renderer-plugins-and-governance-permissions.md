...
插件系统曾同时维护多条 renderer 加载路径以及 Worker + WASM + RPC + 声明式 UI 路径，造成多套激活、能力转发、
UI 和资源生命周期模型。受限路径不能复用 React 生态与完整 SDK，维护成本与插件能力差异持续扩大，而当前产品没有
需要这些路径兼容的插件。仓库中的现有插件已全部使用 Module Federation，因此保留加载模式判别只会制造伪扩展点。
...
1. 插件只使用 Module Federation 加载。`plugin.json` 删除 `runtime` 选择字段，`moduleFederation` 成为必填合同；宿主、
   开发服务器和打包器不再维护其它加载分支。
...
- 插件运行、打包、UI 与能力接入只有一套模型，减少条件分支、默认值、包体和跨运行时协议维护。
