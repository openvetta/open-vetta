...
- `build:presets` 先构建根 workspace 中的 `plugin-sdk`、`plugin-vite`，再逐个构建插件 zip，并将 zip 校验解压到 Desktop 开发 staging；应用开发启动不再直接读取 preset 的 `dist/`。
- preset 进入根 workspace 和根 `bun.lock`；`build:presets` 根据根依赖清单与锁文件按需执行一次冻结安装。
