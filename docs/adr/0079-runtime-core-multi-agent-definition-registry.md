...
- Coding Agent 已通过 execution-compatible `RuntimeAgentDefinition` 接入生产 Composition；独立 Composition 可自建
  `RuntimeAgentRuntime` 控制面，Desktop 使用唯一 `RuntimeHost.agents`。复杂 Session 装配仍保留在产品包，能力只由
  `RuntimeAgentSession` 编译一次；完整所有权收敛见 ADR-0084。
