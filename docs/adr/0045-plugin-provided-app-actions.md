...
8. 首个实现为随包 `vetta-actions` 系统插件，已覆盖全部内置 App Action 领域（含 batch-tasks / scheduler / appearance / navigation）；对应静态 Action 保留为 fallback。插件通过受信任的 `ctx.official` 领域能力读写宿主数据，不反向调用同 id 的静态 Action。
...
- `vetta-actions` 已完成全部内置 App Action 领域的读写迁移（含 batch-tasks / scheduler / appearance / navigation）；静态实现继续作为 fallback。
