...
- 流式期间阶段组**始终收起**，但当前阶段的标题行会投影组内最新活动，避免只显示笼统的「正在处理」：并行调用时优先显示最近的 pending 工具，工具文案依次取 `currentPhase`、agent 写的 `description`、语义化工具名；没有 pending 工具时显示最新 thinking 的单行尾部摘要或最近工具。thinking 摘要按 Unicode 字符截断，完整内容仍只在展开后显示。阶段完成后标题恢复为 `summary` / `label`，历史记录不保留瞬时活动标题。只有最后一个流式 process segment 获得实时标题，避免同一阶段被产物块切开后多行同时显示为活跃。末尾阶段在流式期间不标记为已关闭（`groupBlocksForWork(blocks, customToolNames, streaming)`）。
- `agent_start` 到首个 thinking / tool / text 事件之间由 Assistant 消息自身承载等待态，而不是列表 Footer：Renderer 立即建立带绝对 `startedAt` 的空草稿并显示 Vetta 消息头与格式化耗时；首个活动到达后在同一消息身份上切换到上述阶段投影。切回仍在运行的会话时用 Runtime `currentTurnStartedAt` 恢复草稿，完成耗时也从该消息字段计算，不另设模块级计时真相源。
- 组视图作为**新组件** `ProgressGroupView` / `ProgressGroupRow` 落在 `packages/theme-ui`，现有 `ToolCallGroupView` 原样保留供 coding 与 `apps/site` 使用，两套渲染各自演进。
