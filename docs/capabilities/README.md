...
- Capability Token 已支持以 TypeBox Schema 作为静态类型、运行时 parser 与 JSON Schema 的单一来源，并由 Token 生成不包含执行函数的只读 Catalog。Agent 设置、通用设置、IM 桥接、模型配置、MCP 配置、Webhook、项目、会话、下载、批量任务、应用更新、技能管理、全局快捷键和快捷面板领域已完成迁移；TypeBox 校验错误会转换为稳定 Capability 错误码，`undefined` 输出使用独立的无载荷 Schema，Catalog 会拒绝缺少输入或输出 Schema 的 Token。
...
- Agent 设置、通用设置、IM 桥接、模型配置、MCP 配置、Webhook、项目、会话、下载、批量任务、应用更新、技能管理、全局快捷键和快捷面板领域之外的现有 Capability Token 尚未迁移到 TypeBox Schema，也尚未进入公开 Catalog；后续按领域逐步迁移，并保持静态类型、parser 与 JSON Schema 同源定义。
