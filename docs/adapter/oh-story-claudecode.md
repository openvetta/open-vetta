...
| 原生图像生成/编辑 | 已支持 | RuntimeHost 注入 `generate_image` / `edit_image`，见 [`runtime.ts`](../../apps/desktop-app/src/main/runtime.ts) |
...
desktop 自定义导入虽然递归查找 `SKILL.md`，但只选择一个最浅结果，并只复制该 Skill 的父目录，见 [`findShallowestSkillMd`](../../apps/desktop-app/src/main/ipc/skills.ts)。直接导入整仓最多得到其中一个 Skill，不能得到完整套件。
