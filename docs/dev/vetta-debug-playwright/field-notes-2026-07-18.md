...
复测开始时已经选中 `Vetta Desktop` 主窗口，右侧活动面板处于关闭状态。DOM 中仍存在 `aside`，但其实际尺寸为：
...
同时确认“文件”“浏览器”“Git”三个标签存在，按钮名称变为“关闭活动面板”。结论：选中主窗口之后，打开右侧侧边栏的最短应用内路径是一次点击。

随后用幂等 `run-code` 再执行一次“确保打开”，返回：
...
这避免 Agent 在面板已经打开时再次点击并把它关闭。

需要注意：关闭状态的 snapshot 仍包含侧栏内部文案，只是缺少可交互 ref。仅检查“文件”或“浏览器”文本会产生假阳性，应该断言 `aside` 的 bounding box 宽度大于 `0`。
