...
- AgentCoreTurnEngine 将 JSON Schema 交给 agent-core/AJV；非法模型参数不会进入工具实现。
...
- 覆盖：
  ...
  - `additionalProperties: false` 拒绝非法模型参数，工具实现调用次数保持为 0。
- `packages/runtime-core`
  - `bun run test`
  - 4 个测试文件、21 个测试通过。
- 质量守卫
...
1. 先冻结 Workspace Path Policy 和只读 File Operations Port。
2. 迁移 `read` 的文本读取、截断和取消语义；图片处理作为独立能力后续接入。
3. 迁移 `ls` / `grep` 等只读工具，再迁移 write / edit。
4. 每个工具继续运行 Feature + AgentCoreTurnEngine 合同测试。
