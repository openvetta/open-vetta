...
**软隔离 vs 硬隔离**：
- **图像生成（软）**：`generate_image` / `edit_image` 不因 toggle 关闭而剥离；`imageMode` 只注入隐形意图提示。未开开关时，用户明确说生图/改图仍可调用。
- **知识检索（硬）**：未开「知识检索」时本轮从 tool list **剥离** `kb-read` 工具（`kb_list_available_tags` / `kb_filter_by_tags`）；开启才暴露并注入「优先查知识库」隐形提示。
